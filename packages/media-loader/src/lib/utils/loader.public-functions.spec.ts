...
describe('loader.public-functions', () => {
  ...
  it('infers mime types from extension and media kind', () => {
    expect(
      inferMimeTypeFromExtension({ extension: 'mp4', kind: 'audio' }),
    ).toBe('audio/mp4');
    expect(
      inferMimeTypeFromExtension({ extension: 'mp4', kind: 'video' }),
    ).toBe('video/mp4');
    expect(
      inferMimeTypeFromExtension({ extension: 'webm', kind: 'video' }),
    ).toBe('video/webm');
    expect(
      inferMimeTypeFromExtension({ extension: 'mov', kind: 'video' }),
    ).toBe('video/quicktime');
    expect(
      inferMimeTypeFromExtension({ extension: 'mkv', kind: 'video' }),
    ).toBe('video/x-matroska');
    expect(
      inferMimeTypeFromExtension({ extension: 'avi', kind: 'video' }),
    ).toBe('video/x-msvideo');
    expect(
      inferMimeTypeFromExtension({ extension: 'unknown', kind: 'audio' }),
    ).toBe('audio/mpeg');
    expect(
      inferMimeTypeFromExtension({ extension: 'unknown', kind: 'video' }),
    ).toBe('video/mp4');
  });

  it('classifies media kinds using mime type before extension', () => {
    expect(classifyMediaKind({ mimeType: 'video/mp4', extension: 'mp3' })).toBe(
      'video',
    );
    expect(
    ...
    expect(
      classifyMediaKind({ mimeType: undefined, extension: null }),
    ).toBeNull();
    expect(
      classifyMediaKind({ mimeType: undefined, extension: 'txt' }),
    ).toBeNull();
  });
  ...
  it('detects likely supported media from name and mime', () => {
    ...
    expect(
      isLikelySupportedMedia({ name: 'blob.bin', mimeType: 'audio/ogg' }),
    ).toBe(true);
  ...
  it('checks browser playback support and treats missing document as compatible', () => {
    ...
    expect(
      canBrowserPlayMedia({ mimeType: 'video/x-msvideo', kind: 'video' }),
    ).toBe(false);
  ...
  it('derives default names from loader input kinds', () => {
    ...
    expect(
      fileNameFromInput({ input: { kind: 'url', url: 'https://x/y/z.zip' } }),
    ).toBe('z.zip');
    expect(fileNameFromInput({ input: { kind: 'file', file } })).toBe(
      'track.zip',
    );
    expect(
      fileNameFromInput({
