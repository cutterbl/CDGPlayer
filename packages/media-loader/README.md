...
- The loader performs browser capability checks, but final decode validation for video happens in `@cxing/media-player`.
...
All items below are exported from `@cxing/media-loader` and supported for direct developer use.
...
You can use worker-backed loading via `loadInWorker(...)` with automatic fallback behavior handled by `@cxing/media-player`.
